...
function Test-ChromeConfigured {
    ...
    # Auto-create marker if profile directory exists but marker is missing

    if (-not (Test-Path $markerFile)) {

        if ((Test-Path $profileDir) -and (Test-Path $defaultProfile)) {

            try {

                # Test if profile is properly configured
                ...
                try {
                    ...
                    $null = Get-ChildItem $defaultProfile -ErrorAction Stop

                    $testAccess = $true

                    

                    # Check if Extensions directory exists (created by setup-chrome-profile.ps1)
                    ...
                    if (-not (Test-Path $extensionsDir) -and -not (Test-Path $preferencesFile)) {

                        Write-Verbose "Profile exists but Extensions/Preferences missing - needs recreation"
                    ...
                    Write-Verbose "Profile not accessible, will recreate"

                    $needsRecreation = $true

                }

                if (-not $testAccess -or $needsRecreation) {

                    # Profile corrupted or incomplete - delete and recreate with setup-chrome-profile.ps1

                    Write-Verbose "ChromeProfile needs recreation. Killing Chrome and recreating with proper setup..."

                    

                    # Kill Chrome processes to unlock profile directory
                    ...
                    $setupScript = Join-Path (Split-Path $PSScriptRoot -Parent) "Scripts\setup-chrome-profile.ps1"

                    if (-not (Test-Path $setupScript)) {

                        $setupScript = "C:\MSBMC\Scripts\setup-chrome-profile.ps1"
                    ...
                    if (Test-Path $setupScript) {
                        ...
                        # Delete old profile with elevation (kills Chrome first)
...
"@
                        ...
                        $tempDelete = "$env:TEMP\delete-profile.ps1"

                        $deleteScript | Set-Content $tempDelete -Force

                        $delProc = Start-Process -FilePath "powershell.exe" -ArgumentList "-NoProfile -ExecutionPolicy Bypass -File `"$tempDelete`"" -Verb RunAs -Wait -PassThru

                        Remove-Item $tempDelete -Force -ErrorAction SilentlyContinue

                        

                        # Recreate with setup-chrome-profile.ps1 (elevated)

                        Write-Verbose "Running setup-chrome-profile.ps1..."

                        $createProc = Start-Process -FilePath "powershell.exe" -ArgumentList "-NoProfile -ExecutionPolicy Bypass -File `"$setupScript`"" -Verb RunAs -Wait -PassThru

                        

                        if ($createProc.ExitCode -ne 0) {

                            Write-Verbose "Failed to recreate ChromeProfile"
                        ...
                        Write-Verbose "setup-chrome-profile.ps1 not found"
                    ...
                    # Fix permissions with elevation

                    $fixScript = "icacls '$profileDir' /grant 'msbmc:(OI)(CI)F' /T /Q; exit `$LASTEXITCODE"

                    $tempScript = "$env:TEMP\fix-perms.ps1"

                    $fixScript | Set-Content $tempScript -Force

                    $proc = Start-Process -FilePath "powershell.exe" -ArgumentList "-NoProfile -ExecutionPolicy Bypass -File `"$tempScript`"" -Verb RunAs -Wait -PassThru
                ...
                Write-Verbose "Created missing marker file: $markerFile"

            } catch {

                Write-Verbose "Could not create marker file: $_"
...
if (-not $baseCheck.Success) {
    ...
    foreach ($missingItem in $baseCheck.Errors) {

        Write-Host "  ✗ $missingItem" -ForegroundColor Red
...
if ($vbaudioDone) {
    ...
    if (Test-Path $vbaudioScript) {
        ...
        # Run with elevation (requires Administrator for Register-ScheduledTask)

        Write-Host "Requesting administrator privileges for VB-Audio task creation..." -ForegroundColor Yellow

        $process = Start-Process -FilePath "powershell.exe" -ArgumentList "-NoProfile -ExecutionPolicy Bypass -File `"$vbaudioScript`"" -Verb RunAs -Wait -PassThru

        

        if ($process.ExitCode -ne 0) {

            Write-Host "[ERROR] VB-Audio installation failed (exit code: $($process.ExitCode))" -ForegroundColor Red

            Write-Host "Please run install-vbaudio.ps1 manually as Administrator" -ForegroundColor Yellow
...
if ($watchdogDone) {
    ...
    if (Test-Path $watchdogScript) {
        ...
        Write-Host "Requesting administrator privileges for Watchdog task creation..." -ForegroundColor Yellow

        $process = Start-Process -FilePath "powershell.exe" -ArgumentList "-NoProfile -ExecutionPolicy Bypass -File `"$watchdogScript`"" -Verb RunAs -Wait -PassThru

        

        if ($process.ExitCode -ne 0) {

            Write-Host "[ERROR] Watchdog installation failed (exit code: $($process.ExitCode))" -ForegroundColor Red

            Write-Host "Please run install-watchdog.ps1 manually as Administrator" -ForegroundColor Yellow
...
$s1 = if ($finalResolution) { "[OK]" } else { "[FAIL]" }
$s2 = if ($finalVBAudio) { "[OK]" } else { "[FAIL]" }
$s3 = if ($finalChrome) { "[OK]" } else { "[FAIL]" }
$s4 = if ($finalWatchdog) { "[OK]" } else { "[FAIL]" }
$s5 = if ($finalKiosk) { "[OK]" } else { "[FAIL]" }
...
$null = $Host.UI.RawUI.ReadKey('NoEcho,IncludeKeyDown')
    ...
    Write-Host ""
    Write-Host "========================================" -ForegroundColor Red
    Write-Host "[FAIL] ERROR: Configuration Failed" -ForegroundColor Red
    Write-Host "========================================" -ForegroundColor Red
    Write-Host ""
